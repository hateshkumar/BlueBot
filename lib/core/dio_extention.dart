...
abstract class BlueBotHttpError {
  final int code;
  ...
  factory BlueBotHttpError.fromDioError(DioError e) {
...
class ValidationError {
  ...
  // That way we can return it from BlueBotHttpError.fromDioError
...
// TODO add defaults for non-nullable properties and ? to nullable
class ConflictError extends BlueBotHttpError {
  final dynamic error;
...
// TODO add defaults for non-nullable properties and ? to nullable
class BadRequestError extends BlueBotHttpError {
  final dynamic error;
...
extension ErrorHandling on DioError {
  ...
  bool isBadRequestError() =>
      response?.statusCode == BlueBotHttpError.BAD_REQUEST;

  bool isConflictError() => response?.statusCode == BlueBotHttpError.CONFLICT;

  bool isValidationError() =>
      response?.statusCode == BlueBotHttpError.UNPROCESSABLE;

  bool isNotFoundError() => response?.statusCode == BlueBotHttpError.NOT_FOUND;

  bool isUnauthenticatedError() =>
      response?.statusCode == BlueBotHttpError.UNAUTHENTICATED;

  bool isUnauthorizedError() =>
      response?.statusCode == BlueBotHttpError.UNAUTHORIZED;

  bool isServerError() => response?.statusCode == BlueBotHttpError.SERVER;
  ...
  bool isConnectionError() {
    ...
    return [
      ...
      // - HandshakeException: Connection terminated during turinghake
      DioErrorType.other,
    ].contains(this.type);
  }
